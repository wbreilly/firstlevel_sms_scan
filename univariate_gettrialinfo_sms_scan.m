...
%             tmod = 1;
               % this does run level time modulation but I want sequence
               % level modulation
%             tmod{1} = ones(1,6)*2;
%             tmod{2} = ones(1,6)*2;
%             tmod{3} = ones(1,3)*2;
            ...
            pmod = struct;
            for nmod = 1:3
                pmod.name{nmod} = sprintf('wbr_time_mod_increasing_%d',nmod);
                pmod.param{nmod} = [0:4]';
                pmod.poly{nmod} = 1;
            end
            ...
            %decreasing
            for nmod = 1:3
                pmod(2).name{nmod} = sprintf('wbr_time_mod_decreasing_%d',nmod);
                pmod(2).param{nmod} = [4 3 2 1 0]';
                pmod(2).poly{nmod} = 1;
            end
